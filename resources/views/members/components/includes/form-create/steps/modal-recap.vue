            ...
            <ssf-container>
                ...
                <ssf-row v-for="itemKey in itemKeys" class="py-1">
                    ...
                    <ssf-col size="5" style="font-weight: 600">
                        {{ member.attributes[itemKey].comment ?? itemKey }}
                    </ssf-col>

                    <ssf-col size="7">
                        {{ itemKey === 'activityId' ? activity : member[itemKey] }}
                    </ssf-col>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            ////////// computed
            // @ts-ignore
            const activity = computed(() => find(activities.value, item => item.id === props.member.activityId).label ?? 'N/D')
