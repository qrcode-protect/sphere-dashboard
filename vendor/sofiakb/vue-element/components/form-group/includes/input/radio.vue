<template>
  <div class="ssf-input-container ssf-form-radio">
    ...
    <ssf-input-label
      :label="parent.label"
      :icon="icon"
      :key="`radio-label-${parent.label}`"
      v-if="parent.label"/>

    <!-- TYPE : RADIO -->
    <label
      class="ssf-radio-button"
      v-for="item in parent.items"
      :class="{'ssf-radio-inline': parent.inline}">
      ...
      <input
        ...
        :name="parent.name"
        :value="value(item)"
        :class="{checked: isChecked(item, parent.value)}"
        :required="parent.required"
        :disabled="parent.disabled"
        :checked="isChecked(item, parent.value)"
        @change="$emit('change')"
        @input="onInput(item)"
        ref="inputComponent"
        @focus="$emit('focus')"/>&nbsp;<span>{{ item[parent.field] || item }}</span>
    </label>
